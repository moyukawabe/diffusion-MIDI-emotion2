...
from symbolic_music.cleaned_transformer import CleanedTransformerModel , CleanedTransformerModel_CFG, CleanedTransformerModel_CFG2
...
# 分類器なし・CFGモデルの場合
def create_model_and_diffusion_CFG(
    image_size,
    class_cond,
    learn_sigma,
    sigma_small,
...
def create_model(
    ...
    # ここを使用！
    elif model_arch in ('transformer', 'music-transformer', 'longformer'):  
        attention_ds = []
        for res in attention_resolutions.split(","):
            attention_ds.append(image_size // int(res))
        ...
        return CleanedTransformerModel( # こっち
            in_channels=in_channel,
            model_channels=num_channels,
            out_channels=(out_channel if not learn_sigma else out_channel*2),
            dropout=dropout,
...
def create_model_CFG(
    ...
    # ここを使用
        ...
        return CleanedTransformerModel_CFG( # こっち
            in_channels=in_channel,
            model_channels=num_channels,
            out_channels=(out_channel if not learn_sigma else out_channel*2),
            dropout=dropout,
            ...
            experiment_mode=experiment_mode,
            max_position_embeddings=max(image_size ** 2, 512),
            num_classes=(NUM_CLASSES if class_cond else None) # 追加
        )
    else:
        raise NotImplementedError
    ...
    *,
    ...
    if training_mode == 'e2e': 
        ...
        if use_kl:
            ...
            loss_type = gd.LossType.E2E_MSE # ここに該当
    ...
    return SpacedDiffusion(
        ...
        model_mean_type=(
            ...
            gd.ModelMeanType.PREVIOUS_X if not predict_xstart else gd.ModelMeanType.START_X #240605
        ),
        model_var_type=(
            (
