...
from symbolic_music.scripts.infill_util import create_model , create_embedding 
...
# WandB    
    if ('LOCAL_RANK' not in os.environ) or (int(os.environ['LOCAL_RANK']) == 0):
        wandb.init(
            project=os.getenv("WANDB_PROJECT", "diffusion_lm_finetune"),
...
# データセット用意       
    ...
    if is_midi_task(args):
        # 分類器内の回帰モデルの読み込み : music_attribute
        logger.log('load the MIDI_music_attribute_Linear')
        #args.music_attribute = torch.jit.load(args.music_attribute, map_location="cuda")
    ...
    def get_mapping_func(args, diffusion, data):
        ...
        '''
        model3 = frozen_embedding_model  # +++ 05/01
...
def create_argparser():
    defaults = dict(
        model_path="diffusion_models_base/diff_midi_giant_midi_piano_REMI_bar_block_rand32_transformer_lr0.0001_0.0_4000_sqrt_Lsimple_h128_s2_d0.1_sd102_xstart_midi/model020000.pt", 
        music_attribute="emogen/midi_attribute_Linear_*/traindatamidi_musicat_Linear.pth", 
        data_dir="",
